...
export default function ModuleDroneComponentsView({ onNavigateHome, onNavigatePrev, onNavigateNext, onNavigateAssessment }) {
  ...
  const handleToggleComplete = () => {
    ...
    try {
      if (nextState) {
        localStorage.setItem('asteria_module_mod-drone-components', 'completed');
        localStorage.setItem('learning_mod-drone-components', 'completed');
      } else {
        localStorage.removeItem('asteria_module_mod-drone-components');
        localStorage.removeItem('learning_mod-drone-components');
      }
    } catch (e) {
      console.warn(e);
    }
        ...
        <footer className="pt-8 border-t border-[var(--divider)] flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="w-full sm:w-auto flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
            ...
            >
              ...
              <span>{isCompleted ? 'Module 3 Marked Complete ✓' : 'Mark as Complete'}</span>
            </button>
