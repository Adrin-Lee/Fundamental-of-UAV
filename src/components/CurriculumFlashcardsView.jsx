...
export default function CurriculumFlashcardsView({
  ...
  const [completedMap, setCompletedMap] = useState(() => {
    ...
    try {
      modKeys.forEach(id => {
        if (id === 'mod-types-of-drones') {
          map[id] = localStorage.getItem('asteria_module_mod-types-of-drones') === 'completed' ||
          ...
          map[id] = localStorage.getItem(`asteria_module_${id}`) === 'completed' ||
                    localStorage.getItem(`learning_${id}`) === 'completed';
        }
      });
    } catch {}
    return map;
  ...
  const toggleModuleComplete = (modId, e) => {
    ...
    const nextState = !completedMap[modId];
    setCompletedMap(prev => ({ ...prev, [modId]: nextState }));
    try {
      if (nextState) {
        localStorage.setItem(`asteria_module_${modId}`, 'completed');
        localStorage.setItem(`learning_${modId}`, 'completed');
        ...
        localStorage.removeItem(`asteria_module_${modId}`);
        localStorage.removeItem(`learning_${modId}`);
        if (modId === 'mod-types-of-drones' || modId === 'mod-drone-types') {
  ...
  const getCompletionStatus = (modId) => {
    return Boolean(completedMap[modId]);
  };
                  ...
                  >
                    <div>
                      ...
                      <div className="flex items-center justify-between mb-3">
                        ...
                        {/* Completion Badge / Toggle Button */}
                        <button
                          type="button"
                          onClick={(e) => toggleModuleComplete(mod.id, e)}
                          className={`inline-flex items-center gap-1 font-mono text-[10px] font-bold px-2.5 py-0.5 rounded-full border transition-all cursor-pointer ${
                            isCompleted 
                              ? 'text-[#047857] bg-[#ECFDF5] border-[#A7F3D0] hover:bg-[#D1FAE5]' 
                              : 'text-[var(--text-muted)] bg-[var(--bg-elevated)] border-[var(--divider)] hover:border-[var(--accent-signal)] hover:text-[var(--accent-signal)]'
                          }`}
                          title="Click to toggle Mark as Complete"
                        >
                          {isCompleted ? (
                            <>
                        ...
                        <button
                          ...
                          className={`inline-flex items-center gap-1 px-3 py-1.5 rounded-xl font-mono text-[11px] font-semibold border transition-all cursor-pointer ${
                            isCompleted
                              ...
                              : 'bg-[var(--bg-elevated)] text-[var(--text-secondary)] border-[var(--divider)] hover:border-[var(--accent-signal)] hover:text-[var(--accent-signal)]'
                          }`}
                        >
